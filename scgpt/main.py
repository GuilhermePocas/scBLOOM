...
def run_scGPT(model_name, hyperparameter_defaults, adata, save_dir):
    ...
    #adadata_testata.layers['X_binned'] = adata_test.X.copy()
...
#
#
#    embed_loader = prepare_dataloader(
#        full_data, batch_size=batch_size, shuffle=False
#    )
#
#    cell_embeddings = {}
#    cls_output = {}
#
#    with torch.no_grad():
#        for batch_data in tqdm(embed_loader, desc="Extracting embeddings"):
#            input_gene_ids = batch_data["gene_ids"].to(device)
#            input_values = batch_data["values"].to(device)
#            target_values = batch_data["target_values"].to(device)
#            batch_labels = batch_data["batch_labels"].to(device)
...
#                )
#
#            cls = output_dict["cls_output"]
#            embs = output_dict["cell_emb"]
#
#            # Add to dict
#            for name, emb in zip(batch_cell_names, embs):
#                cell_embeddings[name] = emb.cpu().numpy()  # numpy array for saving
#
#            for name, cls in zip(batch_cell_names, cls):
#                cls_output[name] = cls.cpu().numpy()  # numpy array for saving
#
#    with open(save_dir / "cell_embeddings.pkl", "wb+") as f:
#        pickle.dump(cell_embeddings, f)
#
#    with open(save_dir / "cls_output.pkl", "wb+") as f:
#        pickle.dump(cls_output, f)
