...
class MutualEncoder(torch.nn.Module):
  ...
  def forward(self, x, knn_edge_index, ppi_edge_index):
      ...
      for i in range(self.num_layers):
        ...
        embbded = self.rows_layers[i](embbded, ppi_edge_index) #PPI
      
      return embbded
  
        
class TransformerConvReducrLayer(TransformerConv):
  def __init__(self, in_channels, out_channels, heads=1, dropout=0 , add_self_loops=True,scale_param = 2, bias=10000, **kwargs):
  ...
  def forward(self, x, edge_index, edge_attr=None, **kwargs):
    self._cached_edge_index = edge_index  # store for use in message()
...
class scNET(torch.nn.Module):
  ...
  def calculate_loss(self, x ,knn_edge_index, ppi_edge_index, highly_variable_index, lambda_rows=1, lambda_cols=1):
    ...
    return lambda_rows * row_loss + lambda_cols * (col_loss + reg), row_loss, col_loss
  ...
  def forward(self, x, knn_edge_index, ppi_edge_index):
    ...
    return embbed_rows, embbed_cols, out_features, embbed.T
