...
def load_embeddings(proj_name, dir):
    '''
    ...
    '''
    embedding_dir = os.path.join(dir)

    embedded_genes = load_obj(os.path.join(embedding_dir, f"row_embedding_{proj_name}"))
    embedded_cells = load_obj(os.path.join(embedding_dir, f"col_embedding_{proj_name}"))
    node_features = load_obj(os.path.join(embedding_dir, f"node_features_{proj_name}"))
    ids = load_obj(os.path.join(embedding_dir, f"ids_{proj_name}"))
    out_features = load_obj(os.path.join(embedding_dir, f"out_features_{proj_name}"))

    return embedded_genes, embedded_cells, node_features, out_features, ids

def load_embeddings_embbed(proj_name, dir):
    '''
    Loads the embeddings and gene expression data for a given project.
    ...
    Returns:
        tuple: A tuple containing:
            ...
            - out_features (np.ndarray): Reconstructed gene expression matrix.
    '''
    embeded_genes = load_obj(pkg_resources.resource_filename(__name__,r"/Embedding/" + dir + "/row_embedding_" + proj_name))
    embeded_cells = load_obj(pkg_resources.resource_filename(__name__,r"/Embedding/" + dir + "/col_embedding_" + proj_name))
    #node_features = pd.read_csv(pkg_resources.resource_filename(__name__,r"./Embedding/node_features_" + proj_name),index_col=0)
    node_features = pd.read_pickle(pkg_resources.resource_filename(__name__,r"/Embedding/" + dir + "/node_features_" + proj_name))
    out_features = load_obj(pkg_resources.resource_filename(__name__,r"/Embedding/" + dir + "/out_features_" + proj_name))
    embbed = load_obj(pkg_resources.resource_filename(__name__,r"/Embedding/" + dir + "/embbed_" + proj_name))
    return embeded_genes, embeded_cells, node_features, out_features, embbed